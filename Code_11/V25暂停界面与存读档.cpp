...
void ReDefine()	//重新定义怪物和物品，将被消灭的怪物和被拾取的道具赋值为1，以及出现而未击败的春之BOSS赋值为9998
{
	for(i=8;i<WIDTH;i++)
	{
		for(j=0;j<HIGH;j++)
...
void WriteRecordFile()  //存储游戏数据文件存档
{
	...
	for(i=0;i<6;i++)
	{
		for(j=8;j<WIDTH;j++)
		{
			for(z=0;z<HIGH;z++)
			{
				if( map[i].newcanvas[j][z]!=0 )	//若不为0，则存储重定义值
				{
	...
	i=0;j=20;z=16;
	fprintf(fp,"%d %d %d %d\n",i,j,z,map[i].newcanvas[j][z]);	//防止原本最后一行数据未被读入
	fclose(fp);
}
...
void PauseMenu()	//游戏开始后的暂停菜单界面，按ESC键打开
{
	cleardevice();	//清屏
	settextcolor(WHITE);
	settextstyle(50,0, _T("微软雅黑"));
	outtextxy(WIDTH*12, HIGH*9, _T("1 继续游戏"));
	outtextxy(WIDTH*12, HIGH*12, _T("2 保存档案"));
	outtextxy(WIDTH*12, HIGH*15, _T("3 退出"));
	FlushBatchDraw();
	
	char input;
	if(_kbhit())  // 判断是否有输入
	{
		input = _getch();  // 根据用户的不同输入来执行对应操作
		if (input == '1') 
		{
			start = 1;
			Entrance();
...
void startup()				//数据初始化
{
	...
	FLOOR=3;				//初始位于魔塔外景，视作在第0层
	p.Level=1;				//初始人物等级为1	
	p.Life=1000;			//初始生命值为1000	
	p.Attack=10;			//初始攻击力为10	
	p.Defence=80;			//初始防御力为10
	p.GOLD=0;				//初始持有金币数为0
...
void updateWithoutInput()	//与用户输入无关的更新
{
	...
	else if( player_x==12 && player_y==5 && FLOOR==1 && isCrystal )	//在1层时升至2层的转换
	{
		player_x=9;
		player_y=2;
		FLOOR=2;
	...
	if(canvas[player_x][player_y]==10)
	{
		...
		map[FLOOR].newcanvas[player_x][player_y]=1;	//记录下道具被拾取
		DrawPanel();
		DrawWithoutPlayer();
		DrawPlayer(lastinput);	//重新绘制所有界面
		FlushBatchDraw();
...
void updateWithInput()		//与用户输入有关的更新 
{
	...
	while(_kbhit())
	{
		...
		if(start==0)
		{
			if(input=='1'&&tips==0)	//按下1开始游戏
				start=1;
			else if( input=='2' )	//按下2读档
			{
				ReadRecordFile();
				start=1;
				Entrance();
			...
			else if(input=='3')	//按下3进入提示界面
				tips=1;
		}
		if(tips==1)
		{
			if(input=='z')	//按下z返回初始界面
				tips=0;
		}
		if(OwnExcalibur==0)	//测试用，按n键重新获得圣剑
		{
			if(input=='n')
				OwnExcalibur=1;
		}
		...
		if( input=='w' || input=='s' || input=='a' || input=='d' )
			lastinput=input;	//记录上一次行走按键
		if( dialogue==0 && isbattle==0 )		//在无对话、无战斗情况下进行行走、对话、打开暂停界面判断
		{
			if( input==27 )	//按下ESC键（ACSII码为27），打开暂停界面
			{
				start=3;
			}
			else if( input=='w' )
		...
		else if( dialogue==1 )	//控制与守门人对话继续游戏
		{
			...
			{			
				...
				if( m.uMsg==WM_LBUTTONDOWN )	//点击对话框“是”的部分，守门人和门消失，且失去圣剑
				{
					...
					map[FLOOR].newcanvas[20][15]=1;	//记录下守门人和门消失
					DefineBackground();
					DrawBackground();
					DefineRoad();
					DrawRoad();
